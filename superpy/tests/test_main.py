...
class Test_buy_product:
    def test_buy_product_to_file(self, write_bought_file):
        Bought_keeper(current_path_test_copy).buy_product("Milk", datetime.date(2021,12, 5), 2.5,datetime.date(2021,12,12), [])
        with open(current_path_test_copy, "r", newline="") as file:
            reader = csv.reader(file)
            next(reader)
            rows = [row for row in reader]
            assert rows[-1] == ["1;Milk;2021-12-05;2.5;2021-12-12"]
...
class Test_write_bought_file:
    @pytest.fixture
    def tear_down_bought_file(self):
        yield
