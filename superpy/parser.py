...
import_bought_parser = subparser.add_parser("import", help="import bought from csv")
import_bought_parser.add_argument("path", type=valid_path, help="path to file")


report_parser = subparser.add_parser(
    "report", help="show report for the set date use flag --revenue, --inventory, --profit, --expired"
)
...
report_date_group.add_argument(
    "--now", default=True, dest="report_date", action="store_true", help="Sets date for report as today"
)
report_date_group.add_argument(
    "--yesterday", dest="report_date", action="store_false", help="Sets date for report as yesterday"
)
report_date_group.add_argument(
    "--date", dest="report_date", type=valid_date, action="store", help="sets date for report as as given date format YYYY-MM-DD"
)
